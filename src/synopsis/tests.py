...
class ReferenceCategoryAuditCommandTests(TestCase):
    def setUp(self):
        self.project = Project.objects.create(title="Category Audit Project")
        self.batch = ReferenceSourceBatch.objects.create(
            project=self.project,
            label="Audit batch",
            source_type="journal_search",
        ...
        self.library_reference = LibraryReference.objects.create(
            title="Shared category reference",
            publication_year=2024,
            hash_key="audit-shared",
            reference_folder=["2", "15"],
        )
        self.linked_reference = Reference.objects.create(
            project=self.project,
            batch=self.batch,
            library_reference=self.library_reference,
            hash_key="audit-shared",
            title="Linked project copy",
            unlinked_reference_folder=["3a"],
        )
        self.unlinked_reference = Reference.objects.create(
            project=self.project,
            batch=self.batch,
            hash_key="audit-unlinked",
            title="Unlinked project copy",
            unlinked_reference_folder=["15"],
        )

    def test_audit_reference_categories_reports_unlinked_and_legacy_local_values(self):
        out = io.StringIO()

        call_command("audit_reference_categories", stdout=out)

        output = out.getvalue()
        self.assertIn("Unlinked project references: 1", output)
        self.assertIn(
            "Linked references with legacy local fallback categories: 1",
            output,
        )
        self.assertIn("Unlinked examples:", output)
        self.assertIn("Linked references with local fallback categories:", output)
        self.linked_reference.refresh_from_db()
        self.assertEqual(self.linked_reference.unlinked_reference_folder, ["3a"])

    def test_audit_reference_categories_can_clear_linked_local_values(self):
        out = io.StringIO()

        call_command(
            "audit_reference_categories",
            "--clear-linked-local-categories",
            stdout=out,
        )

        output = out.getvalue()
        self.assertIn("Cleared linked local fallback categories: 1", output)
        self.linked_reference.refresh_from_db()
        self.unlinked_reference.refresh_from_db()
        self.assertEqual(self.linked_reference.unlinked_reference_folder, [])
        self.assertEqual(self.linked_reference.category_values, ["2", "15"])
        self.assertEqual(self.unlinked_reference.unlinked_reference_folder, ["15"])
