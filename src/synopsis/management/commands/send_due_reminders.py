...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        guidance_qs = AdvisoryBoardMember.objects.filter(
            sent_guidance_at__isnull=False,
            feedback_on_guidance_deadline__isnull=False,
            guidance_reminder_sent=False,
            response="Y",
        )
        guidance_qs = guidance_qs.filter(feedback_on_guidance_received__isnull=True)
        guidance_to_remind = []
        for m in guidance_qs:
            deadline = m.feedback_on_guidance_deadline
            if not deadline:
                continue
            if minus_business_days(deadline, lead_days) == today:
                guidance_to_remind.append(m)
        for m in guidance_to_remind:
            subj = email_subject(
                "guidance_reminder", m.project, m.feedback_on_guidance_deadline
            )
            body = (
                f"Dear {m.first_name or 'colleague'},\n\n"
                f"A reminder that guidance feedback for '{m.project.title}' is due by "
                f"{format_deadline(m.feedback_on_guidance_deadline)}.\n\nThank you."
            )
            msg = EmailMultiAlternatives(
                subj, body, to=[m.email], reply_to=reply_to_list(None)
            )
            msg.send()
            m.guidance_reminder_sent = True
            m.guidance_reminder_sent_at = timezone.now()
            m.save(
                update_fields=[
        ...
        self.stdout.write(
            self.style.SUCCESS(
                (
                    ...
                    f"guidance={len(guidance_to_remind)}"
                )
            )
        )
